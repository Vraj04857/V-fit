...
export default function WorkoutPage() {
  ...
  // Derive day focus label from muscle groups of exercises in that day
  const getDayFocusLabel = (exercises, day) => {
    const dayExs = getExercisesForDay(exercises, day);
    if (day === 0 || dayExs.length === 0) return null;
    const groups = [...new Set(dayExs.map(e => e.muscleGroup).filter(Boolean))];
    const pushGroups  = ['chest', 'shoulders', 'triceps'];
    const pullGroups  = ['back', 'biceps', 'traps'];
    const legGroups   = ['quads', 'hamstrings', 'glutes', 'calves'];
    const hasPush   = groups.some(g => pushGroups.includes(g));
    const hasPull   = groups.some(g => pullGroups.includes(g));
    const hasLegs   = groups.some(g => legGroups.includes(g));
    const hasCardio = groups.includes('cardio');
    const hasCore   = groups.some(g => ['abs', 'lowerback'].includes(g));
    if (hasPush && hasPull) return '💪 Upper Body';
    if (hasPush)   return '🫸 Push Day — Chest · Shoulders · Triceps';
    if (hasPull)   return '🫷 Pull Day — Back · Biceps · Traps';
    if (hasLegs)   return '🦵 Leg Day — Quads · Hamstrings · Glutes';
    if (hasCardio) return '🏃 Cardio Session';
    if (hasCore)   return '🔥 Core & Stability';
    return null;
  };
                    ...
                    {isExpanded && (
                      <div style={{ borderTop: '1px solid rgba(44,44,26,0.06)', padding: '16px 24px 20px', animation: 'fadeUp 0.3s ease' }}>
                        ...
                        {dayExercises.length === 0 ? (
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                            {plan.isAutoGenerated && getDayFocusLabel(plan.exercises, selectedDay) && (
                              <div style={{ fontSize: '12px', fontWeight: '600', color: 'var(--green)', background: 'rgba(74,103,65,0.08)', padding: '6px 12px', borderRadius: '8px', marginBottom: '4px' }}>
                                {getDayFocusLabel(plan.exercises, selectedDay)}
                              </div>
                            )}
